import { StudentManager } from "./model/managers/StudentManager.js";
...
document.getElementById("searchEmailBtn").onclick = () => {
    ...
    const found = manager.findStudentByEmail(keyword);
    const results = found ? [found] : [];
    showList(results);
    alert(`พบอีเมล: ${results.length} คน`);
};
